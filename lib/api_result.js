    ...
    for(i=0;i<rowsetElements.length;i++){
      ...
      if(childNodes.length != 0){
        if(childNodes.length ==1 && childNodes[0].name()=='text'){
          ...
          subobject = {};
          api_method['get'+capitalizeFirstLetter(propertyName)] =  function(){return subobject;};
          for(j=0;j<childNodes.length;j++){
            if(childNodes[j].name()!='text'){ 
              subsubobject = {};
              subobject['get'+capitalizeFirstLetter(childNodes[j].name())] =  function(){return subsubobject;};
